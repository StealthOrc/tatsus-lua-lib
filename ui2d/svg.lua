...
local function flatten(points)
    local result = {}
    for _, point in ipairs(points or {}) do result[#result + 1], result[#result + 1] = point[1], point[2] end
    return result
end
...
function Svg.draw(icon, x, y, width, height, tint, alpha)
    if not icon then return false end
    alpha = alpha or 1
    local box = icon.viewbox
    local scale = math.min(width / math.max(1, box.w), height / math.max(1, box.h))
    ...
    for _, item in ipairs(icon.items) do
        ...
        if item.kind == "path" then
            ...
            if item.stroke then
                set_color(item.stroke_color, tint, item_alpha)
        ...
        elseif item.kind == "circle" then
            if item.fill then set_color(item.fill_color, tint, item_alpha); love.graphics.circle("fill", item.cx, item.cy, item.r) end
            if item.stroke then set_color(item.stroke_color, tint, item_alpha); love.graphics.setLineWidth(item.stroke_width); love.graphics.circle("line", item.cx, item.cy, item.r) end
        end
    end
    love.graphics.pop()
    return true
